<?php
class CountrySubdivisionField extends GroupedDropdownField {
	/**
	 * __construct
	 *
	 * @param string $name
	...
	public function __construct($name, $title = null, $source = null, $value = '', $form = null, $emptyString = null) {
		...
		foreach($source as $country) {
			$subdivisions = $country->Subdivisions()->exclude("Name", "");
			if(!$subdivisions->exists())
				continue;
			$parentSource[$country->Title] = $subdivisions->map()->toArray();
		}
